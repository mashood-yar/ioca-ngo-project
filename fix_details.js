...
const ProjectDetails: React.FC<ProjectDetailsProps> = ({ isUrdu }) => {
  ...
  const getStatusDisplay = () => {
    switch (project.status) {
      case 'ongoing':
        return { icon: Clock, textEn: 'Ongoing', textUr: '????', color: 'bg-brand-teal text-white' };
      case 'completed':
        return { icon: CheckCircle2, textEn: 'Completed', textUr: '????', color: 'bg-brand-gold text-brand-navy' };
      case 'upcoming':
        return { icon: Calendar, textEn: 'Upcoming', textUr: '??? ? ??? ??', color: 'bg-purple-600 text-white' };
      case 'paused':
        return { icon: Clock, textEn: 'Paused', textUr: '??? ??? ???', color: 'bg-gray-500 text-white' };
      default:
        return { icon: Clock, textEn: 'Ongoing', textUr: '????', color: 'bg-brand-teal text-white' };
    }
  };
  ...
  return (
    <div className={\min-h-screen bg-brand-gray-light pt-24 pb-16 \\} dir={isUrdu ? 'rtl' : 'ltr'}>
      <SEO 
        title={\\ - IOCA\}
          ...
          <div className="relative h-[30vh] md:h-[50vh] w-full bg-gray-100">
            ...
            <div className="absolute bottom-0 left-0 right-0 p-6 md:p-10">
              <div className="flex flex-wrap items-center gap-3 mb-4">
                <span className={\inline-flex items-center gap-1.5 px-4 py-1.5 rounded-full text-sm font-bold shadow-md \\}>
                ...
                {project.progress !== undefined && project.progress !== null && project.progress > 0 && (
                  <div className="mt-8 pt-6 border-t border-gray-200">
                    <div className="flex justify-between items-center mb-2">
                      <span className="font-bold text-brand-navy text-sm uppercase">{isUrdu ? '??? ???' : 'Progress'}</span>
...
\;
fs.writeFileSync('frontend/src/pages/ProjectDetails.tsx', code);
