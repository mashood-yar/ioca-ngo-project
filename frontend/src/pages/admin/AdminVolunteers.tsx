...
export function AdminVolunteers() {
  ...
  const handleConvertToPersonnel = async (volunteer: Volunteer) => {
    ...
    try {
      ...
      // Update local state so button becomes the converted badge immediately
      setVolunteers(prev => prev.map(v => v.id === volunteer.id ? { ...v, personnel_uid: uid } : v));
