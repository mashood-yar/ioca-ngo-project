...
const Footer: React.FC<FooterProps> = ({ isUrdu }) => {
  // H1-09: Newsletter wiring is pending backend endpoint.
      ...
      <div className="max-w-7xl mx-auto relative z-10">
        <div className="grid grid-cols-1 md:grid-cols-5 gap-8 lg:gap-12 mb-16">
          ...
          <div className="md:col-span-2 flex flex-col items-center md:items-start text-center md:text-left">
            ...
            {/* Social Media — H1-10: Update these URLs once IOCA's profiles are live */}
            <div className="flex gap-3 mt-6 mx-auto md:mx-0">
              {/* TODO (Mashood): Replace href values with real IOCA social URLs */}
              <a href="https://www.facebook.com/ioca.official" target="_blank" rel="noopener noreferrer" aria-label="IOCA on Facebook" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-brand-teal hover:border-brand-teal hover:text-brand-white transition-all">
                <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24" aria-hidden="true"><path fillRule="evenodd" d="M22 12c0-5.523-4.477-10-10-10S2 6.477 2 12c0 4.991 3.657 9.128 8.438 9.878v-6.987h-2.54V12h2.54V9.797c0-2.506 1.492-3.89 3.777-3.89 1.094 0 2.238.195 2.238.195v2.46h-1.26c-1.243 0-1.63.771-1.63 1.562V12h2.773l-.443 2.89h-2.33v6.988C18.343 21.128 22 16.991 22 12z" clipRule="evenodd" /></svg>
              </a>
              <a href="https://www.instagram.com/ioca.official" target="_blank" rel="noopener noreferrer" aria-label="IOCA on Instagram" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-brand-teal hover:border-brand-teal hover:text-brand-white transition-all">
              ...
              <a href="https://www.linkedin.com/company/ioca-official" target="_blank" rel="noopener noreferrer" aria-label="IOCA on LinkedIn" className="w-10 h-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-brand-teal hover:border-brand-teal hover:text-brand-white transition-all">
          ...
          {/* Newsletter — H1-09: Honest CTA while backend endpoint is pending */}
          <div>
            ...
            <div className="flex flex-col gap-3">
              <a
                href="https://www.facebook.com/ioca.official"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3 hover:bg-brand-teal/20 hover:border-brand-teal/40 transition-all group"
              >
                ...
                <span className={`text-sm text-brand-white/80 group-hover:text-brand-white ${isUrdu ? 'font-urduBody' : ''}`}>
                  {isUrdu ? 'فیس بک پر فالو کریں' : 'Follow on Facebook'}
                </span>
              </a>
              <a
                href="https://www.instagram.com/ioca.official"
                target="_blank"
                rel="noopener noreferrer"
                className="flex items-center gap-3 bg-white/5 border border-white/10 rounded-xl px-4 py-3 hover:bg-brand-teal/20 hover:border-brand-teal/40 transition-all group"
              >
                ...
                <span className={`text-sm text-brand-white/80 group-hover:text-brand-white ${isUrdu ? 'font-urduBody' : ''}`}>
                  {isUrdu ? 'انسٹاگرام پر فالو کریں' : 'Follow on Instagram'}
                </span>
              </a>
            </div>
          </div>
